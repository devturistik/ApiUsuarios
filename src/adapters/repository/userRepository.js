...
export default class UserRepository {
  ...
  async createUser(
    ...
    try {
      ...
      const result = await pool
        ...
        .query(
          "INSERT INTO SistemaWebOC.usuarios (clave, nombre, depto, correo, apellido, activo,usuario_creador) OUTPUT INSERTED.Id VALUES (@clave, @nombre, @depto, @correo, @apellido, 0,@usuarioCreador)"
        );
  ...
  async getUserByEmail(email) {
    try {
      ...
      const result = await pool
        ...
        .query(
          "SELECT * FROM SistemaWebOC.Usuarios WHERE correo = @correo and activo = @activo"
        );
  ...
  async updateUser(id, updates) {
    try {
      ...
      const query = `UPDATE SistemaWebOC.usuarios SET ${keys
